...
pub fn renumber(conn: &rusqlite::Connection, root: &PartitionMap, local: &PartitionMap, remote: &PartitionMap) -> Result<DB> {
    ...
    for (name, local_partition) in local.iter() {
        if let Some(remote_partition) = remote.get(name) {
            ...
            if let Some(root_partition) = root.get(name) {
                let root_index = root_partition.next_entid();
                let advanced_locally_by = local_partition.next_entid() - root_index;
                let advanced_remotely_by = remote_partition.next_entid() - root_index;
                if advanced_locally_by > 0 && advanced_remotely_by > 0 {
                    ...
                    let tx_e = "UPDATE timelined_transactions SET e = e + ? WHERE timeline = 0 AND WHERE e >= ? AND e < ?";
                    let tx_a = "UPDATE timelined_transactions SET a = a + ? WHERE timeline = 0 AND a >= ? AND a < ?";
                    let tx_v = "UPDATE timelined_transactions SET v = v + ? WHERE timeline = 0 AND value_type_tag = ? AND v >= ? AND v < ?";
                    let tx_tx = "UPDATE timelined_transactions SET tx = tx + ? WHERE timeline = 0 AND tx >= ? AND tx < ?";
                    ...
                    let next_entid = output.get_mut(name).unwrap().next_entid() + advanced_locally_by;
                    output.get_mut(name).unwrap().set_next_entid(next_entid);
                } else {
                    // Take the largest value. We already defaulted to remote….
                    if local_partition.next_entid() > remote_partition.next_entid() {
                        output.get_mut(name).unwrap().set_next_entid(local_partition.next_entid());
                    }
                }
            } else {
...
mod tests {
    ...
    fn test_renumber_no_change() {
        ...
        local.transact(r#"[[:db/add "e1" :db.schema/version 1]
                           [:db/add "e2" :db.schema/version 2]]"#).expect("transact local 1");

        remote.transact(r#"[[:db/add "e1" :db.schema/version 1]
                            [:db/add "e2" :db.schema/version 2]]"#).expect("transact remote 1");
    ...
    fn test_renumber_local_fast_forward() {
        ...
        local.transact(r#"[[:db/add "e3" :db.schema/version 1]
                           [:db/add "e4" :db.schema/version 2]]"#).expect("transact remote 2");
    ...
    fn test_renumber_remote_fast_forward() {
        ...
        remote.transact(r#"[[:db/add "e3" :db.schema/version 1]
                            [:db/add "e4" :db.schema/version 2]]"#).expect("transact remote 2");
    ...
    fn test_renumber_non_trivial_schema() {
        ...
        rebased.transact(r#"[[:db/add "e1" :db.schema/version 1]
                             [:db/add "e2" :db.schema/version 2]]"#).expect("transact rebased 1");
