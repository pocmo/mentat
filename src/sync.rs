...
fn rollback_savepoint(db_tx: &rusqlite::Transaction, name: String) -> Result<()> {
    db_tx.execute(&format!("ROLLBACK TO {}", name), &[])?;
    Ok(())
}
...
impl Conn {
    pub(crate) fn sync(&mut self,
        ...
        match sync_result {
            ...
            SyncResult::Merge(incoming_txs, mut local_txs_to_merge) => {
                logger::d(&format!("Rewinding local transactions."));
                // 1) rewind local to shared root
                // TODO fold this into "move bunch of transactions to a different timeline"?
                // -> set new (temp) timeline for a set of local transactions going back to root
                // -> somehow update the datoms table and the inProgress schema -> similar to below...
                // -> clean up after ourselves? e.g. wind back the partition, delete retraction transactions... 

                // TODO guarantee sorted order at the interface level?
                local_txs_to_merge.sort();

                let (new_schema, new_partition_map) = timelines::move_from_main_timeline(
                    &in_progress.transaction,
                    &in_progress.schema,
                    in_progress.partition_map.clone(),
                    local_txs_to_merge[0].tx..,
                    1   
                )?;
                match new_schema {
                    Some(schema) => in_progress.schema = schema,
                    None => ()
                };
                in_progress.partition_map = new_partition_map;
                ...
                for remote_tx in incoming_txs {
                    ...
                    for part in remote_tx.parts {
                        if part.added {
                            logger::d(&format!("Add: {:?}", part));
                            builder.add(KnownEntid(part.e), KnownEntid(part.a), part.v)?;
                        } else {
                            logger::d(&format!("Retract: {:?}", part));
                            builder.retract(KnownEntid(part.e), KnownEntid(part.a), part.v)?;
                        }
                    }
                ...
                savepoint(&in_progress.transaction, "speculative_local".to_string())?;

                logger::d(&format!("Transacting local on top of incoming..."));
                // 3) play rewound local txs on top of remote, one by one
                ...
                // This will undo whatever we did in step 3. That includes any changes to datoms, transactions table, and the partitions.
                logger::d(&format!("Rolling back savepoint..."));
                rollback_savepoint(&in_progress.transaction, "speculative_local".to_string())?;
